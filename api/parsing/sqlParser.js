/*
original by David Sferruzza, 2013
https://github.com/dsferruzza/simpleSqlParser
modifed for simple grouping/having
and slimmed down to meet my specific needs.
*/
(function (exports) {
    ...
    exports.sql2ast = function (query) {
        ...
        var parts_name = ['SELECT', 'FROM', 'DELETE FROM', 'INSERT INTO', 'UPDATE', 'ORDER BY', 'GROUP BY', 'HAVING', 'WHERE', 'LIMIT', 'VALUES', 'SET'];
        ...
        analysis['GROUP BY'] = function (str) {
            ...
            var result = [];
            str.forEach(function (item, key) {
                var group_by = /([A-Za-z0-9_\.]+)\s*(\(([A-Za-z0-9_\., ]+)\))?/gi;
    ...
    exports.ast2sql = function (ast) {
        ...
        if (typeof ast['SELECT'] != 'undefined' && typeof ast['FROM'] != 'undefined') {
            result = select(ast) + from(ast) + where(ast) + order_by(ast) + limit(ast);
        }
